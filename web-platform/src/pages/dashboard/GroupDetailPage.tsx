...
import { type AcademicGroup, subscribeToPosts, createPost, type GroupPost } from "../../lib/groups";
import { ArrowLeft, Users, User, Shield, MessageSquare, Send } from "lucide-react";
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b pb-6">
          <div>
            ...
            <p className="text-lg text-gray-600 mt-1">{group.code} • {group.lecturerName}</p>
          </div>
          
          <div className="flex items-center gap-4 bg-gray-50 p-3 rounded-lg border">
             <div className="flex flex-col text-right">
                <span className="text-xs uppercase text-gray-500 font-semibold tracking-wider">Join Code</span>
                <span className="font-mono text-xl font-bold text-indigo-600 select-all">{group.joinCode}</span>
             </div>
             <div className="h-8 w-px bg-gray-300"></div>
             <div className="flex flex-col">
                <span className="text-xs uppercase text-gray-500 font-semibold tracking-wider">Members</span>
        ...
        <div className="lg:col-span-2 space-y-6">
           ...
           <div className="bg-white rounded-lg border shadow-sm p-4">
              <form onSubmit={handlePost} className="flex gap-4">
                <div className="flex-1">
                  <input 
                    type="text" 
                    placeholder={isRep ? "Post an announcement..." : "Ask a question or share something..."}
           ...
           <div className="space-y-4">
              {postsLoading ? (
                ...
                posts.map((post) => (
                  <div key={post.id} className="bg-white rounded-lg border shadow-sm p-4 transition-all hover:bg-gray-50/50">
                     <div className="flex items-start justify-between mb-2">
                        ...
                        <span className="text-xs text-gray-500">
                          {post.createdAt?.toMillis ? formatDistanceToNow(post.createdAt.toMillis(), { addSuffix: true }) : 'Just now'}
                        </span>
                     </div>
        ...
        <div className="space-y-6">
           <div className="bg-white rounded-lg border shadow-sm p-4">
              ...
              <div className="text-sm text-gray-600">
                 {isRep ? "You are managing this class." : "Contact your Rep for info."}
              </div>
           </div>
        </div>
      </div>
